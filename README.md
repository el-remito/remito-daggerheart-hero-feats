...
- **“Hide always” still means always.** A Feat carrying one entry set to hide outright is hidden whatever its other tags say, and the per-Feat **Hidden** switch is unchanged and absolute — it remains the way to hold a Feat back for reasons no rule can see, released by unticking it
- **A Feat withheld until met that states no prerequisite now says so.** It can never appear, so the registry row carries a warning rather than leaving it to be noticed by its absence
- **The Unreachable Feats audit now reports what it left out.** Secret Feats are not counted as supply, because a player cannot reach one today — but a Category that passes only because they were excluded is not the same as one that passes outright, and the panel now says how many and why
- **An UPDATED chip, in its own colour.** When a Feat’s Level or requirements change after it has been published, players see that it is not what it was — the ten most recently changed, counted separately from the ten most recently added, so a busy week of filing cannot crowd them out. Curating a Feat can never mark it changed: until you first Save it, or File it from the Curation queue, it is being written, and it reads **NEW**
- **Mark as New / Updated, by hand.** Editing a Feature’s own actions in a compendium changes what a Feat does without touching anything the registry can see, so each expanded Feat row carries two toggles to say so yourself — saved on Save and undone by Discard, like every other field there
- **Statistics remembers which characters you set aside.** The Table adoption excludes survive closing the window and reloading. Stored on the world rather than per user, because “this sheet is not real play data” is true for whoever opens the tab
- **Most played Categories.** A new bar list above Most taken, showing where the counted characters’ Feats actually go — a Category can be the most played while no single Feat in it is the most taken. It moves with the exclude toggles like every other figure there
- **Recent acquisitions dates read DD/MM/YYYY.** They followed whichever locale the browser happened to be in, so the same world could show two different orders on two machines — and 01/02 against 02/01 gives a reader no way to tell which they are looking at
- **“Newly added Feats” moved up the filter rail**, to sit beside “Only feats I qualify for” under Level, in the player catalog and the GM’s Feats tab alike. It sat below Category, at the far end of the rail from the switch it is read with — both narrow *which Feats do I want to look at*, while Type and Category describe what a Feat *is*
### v1.5.3
...
- **It audits requirements you wrote by hand, too**, and runs whether or not Rule Automation is switched on. Before, a Feat carrying its own Investment in Category row opted out of the rule and was therefore never checked at all — the likelier place for the mistake, since the curve is applied uniformly by machine while authored numbers are typed one at a time. Chains using **and** / **or** and rows naming a different Category are handled, and each finding is spelled out in the same words the player sees
- **Feats withheld from players are no longer counted as investment.** A hidden Category or a hidden Type withholds a Feat exactly as the per-Feat flag does, so none of them can be somebody else’s prerequisite
- **"Newly added Feats".** A new filter in both rails — your players’ Feats window and the Registry’s Feats tab — showing the ten Feats you curated most recently. Those ten also wear a gold **NEW** chip wherever they appear, so a player browsing normally sees what has just arrived without touching the filter. Purely a signal: it grants nothing and gates nothing
- **A third tab in a player’s Feats window: "My Investments".** Where you stand in each Category you have Feats in, what the next mark is, how far off it is, and how many Feats it unblocks — the Investment requirement read from the player’s end instead of the Feat’s. The marks come from the Feats themselves rather than from the Automation curve, so the tab is right whether the rule is on or off and never promises a tier no Feat actually gates
- **Statistics → Table adoption: leave a character out of the figures.** A toggle on each row of the ledger drops that character from Most taken, Recent acquisitions and the counters — for retired heroes, NPC sheets and test dummies skewing the numbers. The row stays in the table, greyed, so you can put them back, and nothing about their own Feats or points changes. Like everything on this tab it is worked out fresh each time and stored nowhere, so it resets when you close the Registry
